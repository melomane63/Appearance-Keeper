...
export default class AppearanceKeeperPreferences {
    ...
    fillPreferencesWindow(window) {
        ...
        try {
            this._settings = new Gio.Settings({
                schema_id: 'org.gnome.shell.extensions.appearance-keeper'
            });
    ...
    _loadCSS() {
        const css = `
            ...
            .accent-circle-default { color: #1c71d8 }
            .accent-circle-blue { color: #1c71d8 }
            .accent-circle-green { color: #2ec27e }
            .accent-circle-yellow { color: #e5a50a }
            .accent-circle-orange { color: #ff7800 }
            .accent-circle-red { color: #c01c28 }
            .accent-circle-purple { color: #813d9c }
            .accent-circle-pink { color: #e66197 }
            .accent-circle-slate { color: #52606d }
        `;
...
This is a preview of your configuration. Change it using the GNOME tools.`);
    ...
    _createModeSection(modeTitle, mode, wallpaperKey) {
        const modeBox = new Gtk.Box({
            ...
            spacing: 8,
            hexpand: true,
            margin_top: mode === 'light' ? 8 : 0,
            margin_bottom: mode === 'dark' ? 12 : 4
        });
    ...
    _createParamsList(mode) {
        ...
        settings.forEach(key => 
            paramsBox.append(this._createParamRow(
    ...
    _createAccentCircle(colorValue) {
        ...
        circleLabel.get_style_context().add_provider(
            cssProvider, 
            Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION
        );
    ...
    _connectSignals() {
        ...
        extensionKeys.forEach(key => {
            const handlerId = this._settings.connect(
                `changed::${key}`, 
                () => this._updateValueLabel(key)
            );
            this._handlers.push({ settings: this._settings, id: handlerId, key });
        });

        ['picture-uri', 'picture-uri-dark'].forEach(key => {
            const handlerId = this._backgroundSettings.connect(
                `changed::${key}`, 
                () => this._updateWallpaperDisplay(key)
            );
            ...
            if (type === 'accent-color') {
                ...
                if (currentValue !== 'Default') {
                    if (existingCircle) {
                        ...
                        Object.keys(this._accentColors).forEach(color => 
                            circleStyleContext.remove_class(`accent-circle-${color}`)
                        );
                        ...
                        cssProvider.load_from_data(
                            `.accent-circle-${currentValue} { color: ${colorHex} }`, 
                            -1
                        );
                        
                        circleStyleContext.add_provider(
                            cssProvider, 
                            Gtk.STYLE_PROVIDER_PRIORITY_APPLICATION
                        );
